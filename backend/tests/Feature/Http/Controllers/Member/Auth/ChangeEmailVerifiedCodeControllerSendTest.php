...
class ChangeEmailVerifiedCodeControllerSendTest extends TestCase
{
    ...
    public function test_send_success(): void
    {
        ...
        Mail::assertSent(ChangeEmailVerifiedCodeMail::class, function ($mail) use ($email) {
            return $mail->hasTo($email);
        });
        // 指定のメールが1回送信されていること
        Mail::assertSent(ChangeEmailVerifiedCodeMail::class, 1);
    ...
    public function test_send_failed_by_validation_error(array $requestBody, array $expectedError): void
    {
        ...
        $response = $this->postJson('/member-auth/change-email-verified-code/send', $requestBody);
    ...
    public function test_send_failed_by_member_already_registered(): void
    {
        ...
        // 認証コードメールが送信されていないこと
        Mail::assertNotSent(ChangeEmailVerifiedCodeMail::class);
